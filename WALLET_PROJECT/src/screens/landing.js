...
import { geckoNative, geckoTokens, NODE_ENV_NETWORK_RCP, NODE_ENV_CHAINLINK_FEED_CONTRACT, NODE_ENV_DATA_FEEDS_RCP, tokens, tokensContracts, contentColor, native } from "../../env"
...
import ausd from "../assets/ausd-token-icon.png";
...
class Landing extends Component {
    constructor(props) {
        ...
        this.state = {
            // Price Feeds
            prices: [],
            symbol: ["AVAX", "BNB", "BTC", "DOT", "ETH","ACA", "LINK", "MATIC", "NEO", "SOL", "USDC", "aUSD", "XRP"],
            icons:  [avax  , bnb  , btc  , dot  , eth  , acala , link  , matic  , neo  , sol   , usdc  , ausd , xrp  ],
        };
        reactAutobind(this)
    ...
    async updatePriceFeeds() {
        ...
        // To be improve
        const geckoNatTok = [geckoNative].concat(geckoTokens)
        const natTok = [native].concat(tokens)
        let results = await getUSD(geckoNatTok)
        let tokenUSD = {}
        geckoNatTok.forEach((item, index) => tokenUSD[natTok[index]] = results[item].usd)
